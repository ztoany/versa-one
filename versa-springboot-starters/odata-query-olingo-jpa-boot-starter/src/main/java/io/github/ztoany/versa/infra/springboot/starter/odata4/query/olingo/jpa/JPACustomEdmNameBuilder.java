...
public record JPACustomEdmNameBuilder(@Nonnull String namespace, @Nonnull boolean propNameFirstToLower) implements JPAEdmNameBuilder {
    ...
    public String buildEntityTypeName(final EntityType<?> jpaEntityType) {
        EdmAlias alias = jpaEntityType.getJavaType().getAnnotation(EdmAlias.class);
        if (alias == null) {
    ...
    public final String buildPropertyName(final String jpaAttributeName) {
        return propNameFirstToLower ? firstToLower(jpaAttributeName) : firstToUpper(jpaAttributeName);
    }
